...
export class IndeedScraper {
  ...
  private processJob(job: any): Job {
    ...
    const compensation = job.compensation?.baseSalary
      ? `${job.compensation.baseSalary.range.min} - ${job.compensation.baseSalary.range.max} ${job.compensation.currencyCode}`
      : null;
    const datePosted = new Date(job.datePublished / 1000)
      .toISOString()
      .split("T")[0];
    ...
    return {
      ...
      companyUrl: job.employer
        ? `https://www.indeed.com${job.employer.relativeCompanyPageUrl}`
        : null,
  ...
  private isJobRemote(job: any, description: string): boolean {
    ...
    return (
      remoteKeywords.some((keyword) =>
        description.toLowerCase().includes(keyword)
      ) ||
      job.attributes.some((attr: any) =>
        remoteKeywords.includes(attr.label.toLowerCase())
      )
    );
  }
}
