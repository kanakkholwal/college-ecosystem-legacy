...
export function TabsTransitionPanel({ items, className,defaultActiveIndex, classNames }: TransitionPanelProps) {
    ...
    return (
        <div className={cn("space-y-4", className)}>
            ...
            <div className={cn("overflow-hidden border-t border-zinc-200 dark:border-zinc-700", classNames?.tabContentList)}>
                <TransitionPanel
                ...
                >
                    {items.map((item) => (
                        <div key={item.id} className={cn("mt-2 ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2", classNames?.tabContent)}>{item.content}</div>
                    ))}
                </TransitionPanel>
            </div>
        </div>
