...
export const NotFoundSvg = (props: React.ComponentProps<"svg">) => {
    return <svg
        viewBox="0 0 480 360"
        xmlns="http://www.w3.org/2000/svg"
        aria-label="404"
        {...props}
        className={cn("w-full h-80 my-4", props.className)}
    >
...
export default function NotFoundContainer({
    ...
    return (
        <div className={`w-full h-full relative my-10 mx-auto flex items-start justify-center gap-6 px-3 flex-col lg:flex-row mt-20 max-w-(--max-app-width) ${className}`}>
            <main className="w-full">
                <section className="flex items-center justify-center w-full gap-20 flex-col-reverse lg:flex-row">
                    <div className="space-y-4 lg:space-y-6 max-w-md text-center lg:text-left">
                        <h1 className="text-xl lg:text-4xl font-semibold lg:font-bold">
                            {title || "Sorry, No Page Found!"}
                        </h1>
                        ...
                        <div className="flex gap-3 justify-center lg:justify-start pt-8">
                            <PreviousPageLink />
                            <ButtonLink rounded="full" variant="default_light" href="/" {...props}>
                                {children ? children : <>
                                    <Home />
                                    Go to Home
                                </>}
                    ...
                    <NotFoundSvg
                        ...
                        className="max-w-sm lg:max-w-md w-full pointer-events-none select-none"
                    />
                </section>
            </main>
        </div>
