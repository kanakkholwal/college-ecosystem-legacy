...
export async function generateEventsByDoc(
      ...
      content: [
        ...
        ...(images.map((url) => ({
          ...
          image:
            typeof url === "string"
              ? new URL(url)
              : new URL(URL.createObjectURL(new Blob([url]))),
  ...
  try {
    const response = await generateObject({
      ...
      schemaName: "events",
      schemaDescription:
        "Array of events extracted from the document, each event must follow the events schema.",
    ...
    if (NoObjectGeneratedError.isInstance(error)) {
      return Promise.resolve({
        ...
        message:
          error.text ||
          "No events were generated from the document. Please ensure the document contains clear and structured event information.",
      });
    }

    // Handle other types of errors
    console.error(
      "An error occurred while generating events from the document:",
      error
    );
    return Promise.resolve({
      error:
