...
export async function mongoToPgDatabase(
  ...
  try {
    ...
    for await (const user of mongoUsers) {
      if (
        user.rollNo === "21dec026" ||
        user.email === "kanakkholwal@gmail.com"
      ) {
        continue;
      }
      ...
      if (existingUser.length > 0) {
        console.log(
          `User with email ${user.email} already exists. Skipping migration.`
        );
        continue;
      }
      ...
      await auth.api.signUpEmail({
        body: {
          ...
          department:
            user.department === "Mathematics & Computing"
