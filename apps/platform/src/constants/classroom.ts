...
import { z } from "zod";
import { rollNoSchema } from "./departments";
import { emailSchema } from "./user";


export const classRoomSchema = z.object({
    className: z.string().min(1, { message: "Class name is required" }),
    department: z.string().min(1, { message: "Department is required" }).default("mix"),
    year: z.string().min(1, { message: "Year is required" }),
    students: z.array(z.object({
        name: z.string().min(1, { message: "Student name is required" }),
        rollNo: rollNoSchema.or(z.string().min(1, { message: "Roll number is required" })),
