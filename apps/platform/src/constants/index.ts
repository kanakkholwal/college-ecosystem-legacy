...
export const passwordSchema = z
    ...
    .refine(
      (password) =>
        (password.match(passwordSettings.uppercaseRegex) || []).length >= passwordSettings.minUppercase,
      {
    ...
    .refine(
      (password) =>
        (password.match(passwordSettings.lowercaseRegex) || []).length >= passwordSettings.minLowercase,
      {
        message: `Password must contain at least ${passwordSettings.minLowercase} lowercase letter`,
      }
    )
    .refine(
      (password) => (password.match(passwordSettings.numbersRegex) || []).length >= passwordSettings.minNumbers,
      {
    ...
    .refine(
      (password) =>
        ...
        (password.match(passwordSettings.specialChars) || []).length >= passwordSettings.minSpecialChars,
      {
