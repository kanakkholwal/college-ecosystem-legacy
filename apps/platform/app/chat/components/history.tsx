...
export default function ChatHistory() {
    const [chats, setChats] = useLocalStorage<ChatListType[]>('chats_history', []);
...
function ChatButton({ chat, style }: ChatButtonProps) {
    ...
    return <Link  href={`chat.id=${chat.id}`}
        className={cn(
        ...
        style={style}>
        <span className="text-sm font-semibold whitespace-nowrap">{chat.title}</span>
