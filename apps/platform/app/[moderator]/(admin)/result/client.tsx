...
import type { ResultType, rawResultSchemaType, rollNoSchemaType } from "~/lib/server-apis/types";
    ...
    const handleGetResult = async () => {
        ...
        try {
            ...
            if (method === "getResultByRollNoFromSite") {
                const { data: response } = await serverApis.results.getResultByRollNoFromSite(rollNo)
                ...
                const { data: response } = await serverApis.results.getResultByRollNo(rollNo)
                if (response?.error || !response?.data) {
                ...
                const { data: response } = await serverApis.results.addResultByRollNo(rollNo)
                if (response?.error || !response?.data) {
                ...
                const { data: response } = await serverApis.results.updateResultByRollNo([rollNo as rollNoSchemaType, {}])
                if (response?.error || !response?.data) {
                    toast.error(response?.message || "Failed to update result by Roll No");
                    return;
                }
                setResult(response.data);
            }
            toast.success(`Result fetched successfully using ${changeCase(method, "camel_to_title")}`);
