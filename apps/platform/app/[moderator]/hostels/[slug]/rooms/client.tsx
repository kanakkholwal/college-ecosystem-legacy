    ...
    const processData = () => {
        ...
        const processedData = data.map((row) => {
            ...
            const seater = Number.parseInt(
                wordsToNumbers(
                    (row[seaterKey] !== null && row[seaterKey] !== undefined ? row[seaterKey].toString() : "one").split(" ")?.[0].toLowerCase()
                )?.toString() || "1"
            );
            return {
    ...
    return (<div className="my-5">
        <ExcelFileHandler
            callBackFn={async (data) => {
                const filteredRowData = filterRowsByCallback<string[]>(data, (row) => {
                    return row.length > 0
                        && row[0] !== null &&
                        row[0] !== undefined
                        && row.filter((cell) => cell !== "").length > 2
                });
                const filteredData = filterColumnsByCallback<string[]>(filteredRowData, (cell) => {
