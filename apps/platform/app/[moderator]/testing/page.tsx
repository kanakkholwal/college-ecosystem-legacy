        ...
        {extractedKeys.length > 0 && (<div className="flex w-full items-center gap-3 flex-wrap mt-5">
            <div className="grid w-full items-center gap-2.5 grid-cols-1 @lg:grid-cols-3">
                {extractedKeys.map((key) => {
                    return (<div className="grid w-full max-w-sm items-center gap-1.5" key={key}>
                        <Label htmlFor={key}>
                            {key}
                        </Label>
                        ...
                        <Select onValueChange={(value) => console.log(value)}>
                            ...
                            <SelectContent>
                                {data.map((row, rowIndex) => {
                                    return (<SelectItem key={`${key}-${rowIndex.toString()}`} value={row[rowIndex]}>{row[rowIndex]}</SelectItem>)
                                })}
                            </SelectContent>
                        </Select>
