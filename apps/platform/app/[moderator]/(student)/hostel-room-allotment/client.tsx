        ...
        <>
            <ResponsiveDialog
                ...
                description={<>
                    ...
                    <span className={`text-sm font-semibold ${(room.occupied_seats >= room.capacity) || room.isLocked ? "text-red-500" : "text-green-500"}`}>
                        {room.isLocked ? "Room is locked" : (room.occupied_seats >= room.capacity) ? "Room is full" : "Room is Joinable"}
                    </span>
                </>}
            >
                {room?.hostStudent ? <div className="flex flex-col items-center justify-center p-4 space-y-2">
                    <div>
                        {typeof room.hostStudent === "string" ? "Room has a host" : `
                                Host : ${room.hostStudent.name} $({room.hostStudent.email})
                            `}
                    </div>
                    <div className="flex flex-row items-center justify-start gap-1">
                        <span className="text-muted-foreground text-sm">Joined students : </span>
                        {(typeof room.hostStudent !== "string" ? roomInfo.data?.members?.concat([{
                            ...
                            <Badge key={member.rollNumber} variant="outline" className="rounded-xl px-1.5">
                                <span className="text-xs">{member.rollNumber} ({member.name})</span>
                                {room.hostStudent === hostId && (member.email === roomInfo.data.hostStudent.email) && (<button
                                ...
                                >
                                    <span className="sr-only">Remove {member.name} option</span>
