export const content = {
  headline: "Empowering Education Through Open Source",
  description:
    " We're building the future of educational technology with a community-driven platform that makes learning accessible, engaging, and effective for students and educators worldwide.",
  wiki_url:
    "https://github.com/kanakkholwal/college-ecosystem/wiki/Introduction",
  contributing_url:
    "https://github.com/kanakkholwal/college-ecosystem/blob/main/CONTRIBUTING.md",
  visitors: 40_400,
};
