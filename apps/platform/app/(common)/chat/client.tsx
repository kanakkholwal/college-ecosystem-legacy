    ...
    return (
        <main className="w-full min-h-full relative flex flex-col justify-between items-start gap-2">
            <div className="w-full border-b pb-2 flex items-center justify-between">
                {editingChatName ? (<div className="flex items-center gap-2">
                    <Input
                        value={chat.title}
                    ...
                    />
                    <X className='cursor-pointer size-4 hover:text-primary' onClick={() => setEditingChatName(false)} />
                </div>
                ) : (
                    <span className="text-sm text-muted-foreground font-medium inline-flex gap-2 items-center">
                        {chat.title || 'Chat with AI Assistant'}
                        <Edit2 className='cursor-pointer size-3 hover:text-primary' onClick={() => setEditingChatName(true)} />
                    </span>
                )}
            ...
            <div className="flex-1 overflow-y-auto w-full max-h-[60vh] space-y-2 p-2">
                ...
                {messages.map(m => (
                    <div key={m.id} className={cn(
                        ...
                        m.role === 'user' ? "flex-row-reverse ml-auto" : "flex-row mr-auto",
                    )}>

                        <Avatar className="shrink-0 size-6">
                            <AvatarImage src={m.role === 'user' ? 'https://cdn-icons-png.flaticon.com/512/17701/17701286.png' : 'https://cdn-icons-png.flaticon.com/512/16921/16921758.png'} />
                            <AvatarFallback>
                                {m.role === 'user' ? 'U' : 'A'}
                            </AvatarFallback>
                        </Avatar>
