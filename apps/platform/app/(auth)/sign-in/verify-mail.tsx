...
export default function VerifyEmail() {
    ...
    const token = (searchParams.get("token") ?? "").trim().length > 0 ? searchParams.get("token") : null;
        ...
        if (token) {
            ...
            authClient.verifyEmail({
                ...
                if (res.error) {
                    ...
                    setError(res?.error?.message ?? "An error occurred while verifying your email.");
                }else{
                    setVerified(true);
                }
            }).catch(e =>{
    ...
    return (
        <main className="flex flex-col items-center justify-center w-full space-y-4">
            <ConditionalRender condition={!!token}>
                ...
                <CardDescription>
                    {isVerifying ? "Please wait while we verify your email." : verified ? "Your email has been verified successfully." : "An error occurred while verifying your email."}
                </CardDescription>
