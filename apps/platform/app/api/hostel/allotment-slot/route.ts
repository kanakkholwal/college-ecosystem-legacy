...
export async function GET(request: Request) {
    try {
        ...
        if (!hostelId) {
            return NextResponse.json({ message: "No hostel id provided" }, { status: 400 });
        }
        ...
        const data = [
            ['Slot Number', 'Slot Timing', 'Roll Number', 'Name']
        ];

        // Fetch student details for each slot
        const allSlots = await Promise.all(upcomingSlots.map(async (slot, index) => {
            ...
            return {
                slotNumber,
                slotTiming,
                slotRollNumbers: slotRelatedStudents.map(student => student.rollNumber),  // Fix incorrect field
        ...
        return NextResponse.json(
            { message: "Failed to generate Excel file", error: (error as Error).message },
            { status: 500 }
        );
    }
}
