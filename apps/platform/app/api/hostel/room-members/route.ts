...
export async function GET(request: Request) {
    try {
        ...
        const roomMembers = await RoomMemberModel.findById(roomId).populate('hostStudent', 'name rollNumber email');
        ...
        const data = {
            ...
            members: roomMembers.map((member: { name: string; rollNumber: string; email: string; }) => ({
                name: member.name,
