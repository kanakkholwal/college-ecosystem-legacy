                ...
                <div className="bg-card rounded-2xl shadow p-4">
                    ...
                    <div className="text-sm grid grid-cols-1 gap-1">
                        {CATEGORIES.map((category) => {
                            return (
                                <Link key={category.value} href={`/community?c=${category.value}`} className="hover:bg-muted hover:text-foreground cursor-pointer py-1 px-2 rounded-lg inline-flex items-center gap-1.5 text-muted-foreground text-sm">
                                    <Image
                                        src={category.image}
