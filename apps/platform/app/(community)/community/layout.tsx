...
import { ArrowRight, SortAsc, TrendingUpIcon } from "lucide-react";
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 min-h-screen">
            ...
            <aside className="hidden md:block md:col-span-1 space-y-4 sticky top-4 h-fit">
                <div className="bg-card rounded-2xl shadow p-4">
                    <h2 className="text-base font-semibold mb-2">All Communities</h2>
                ...
                <div className="bg-card rounded-2xl shadow p-4">
                    ...
                    <div className="text-sm grid grid-cols-1 gap-1">
                        {CATEGORIES.map((category) => {
                            return (
                                <Link key={category.value} href={`/c/${category.value}`} className="hover:bg-muted hover:text-foreground cursor-pointer py-1 px-2 rounded-lg inline-flex items-center gap-1.5 text-muted-foreground text-sm">
                                    <Image
                                        src={category.image}
