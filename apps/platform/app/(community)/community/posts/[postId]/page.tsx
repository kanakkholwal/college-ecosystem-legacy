...
export default async function CommunityPost({ params }: CommunityPostProps) {
    ...
    const post = await getPostById(params.postId, cache.has(params.postId) || false);
                    ...
                    <div className="flex flex-wrap items-center w-full mb-4">
                        <div className="flex gap-2">
                            <Avatar className="bg-gray-200 shadow">
                                <AvatarImage src={CATEGORY_IMAGES[post.category]} alt={post.category} />
                                <AvatarFallback>{post.category[0].toUpperCase()}</AvatarFallback>
                            </Avatar>
